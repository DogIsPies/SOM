...
public class SOM {
	...
	public void uczKolo(Vec2D wejscia){
		int idxW=0,idxK=0;
		double minDist=distCircle(neurony[idxW][idxK],wejscia);
		for(int i=0;i<neurony.length;i++)
			for(int j=0;j<neurony[i].length;j++){
				double dist=distCircle(neurony[i][j],wejscia);
				if(dist<minDist){
					idxW=i;
					idxK=j;
					minDist=dist;
	...
	public double distCircle(Vec2D a, Vec2D b) {
